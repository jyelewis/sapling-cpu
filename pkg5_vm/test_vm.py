...
def test_add_carry_last_without_prior_carry_is_plain_add():
    # Plain ADD on low bytes does not set CF; high-byte ADD with CARRY_LAST
...
def test_sub_carry_last_chains_16bit_diff():
    # (R1:R0) = 0x0100 − 0x0001 = 0x00FF.
    # Low: 0x00 - 0x01 = -1 → R0=0xFF, CF=1 (borrow).
    # High with CARRY_LAST: 0x01 - 0x00 - 1 = 0x00 → R1=0x00, no final borrow.
    cpu = run_source("""
        LD R0 0x00
        LD R1 0x01
        LD R2 0x01
